...
import { tools as initialTools } from "@/lib/mock/data";
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground max-w-xl">
          Set a USDC price per MCP tool. Free tools stay open. Paid tools issue
          a 402 challenge before execution.
        </p>
      </div>
        ...
        <table className="w-full text-sm min-w-[720px]">
          <thead>
            <tr className="hairline-b text-muted-foreground">
              <th className="text-left px-4 py-3 font-normal text-xs uppercase tracking-wider">Tool</th>
              <th className="text-left px-4 py-3 font-normal text-xs uppercase tracking-wider">Description</th>
              <th className="text-right px-4 py-3 font-normal text-xs uppercase tracking-wider">Price</th>
              <th className="text-right px-4 py-3 font-normal text-xs uppercase tracking-wider">24h calls</th>
              <th className="text-center px-4 py-3 font-normal text-xs uppercase tracking-wider">Status</th>
              ...
              <tr key={t.id} className="hairline-b last:border-b-0 hover:bg-card/40">
                ...
                <td className="px-4 py-3 text-right font-mono">
                  {editingId === t.id ? (
                    ...
                    >
                      {t.priceUsdc === 0 ? <span className="text-muted-foreground">free</span> : fmtUsdc(t.priceUsdc)}
                    </button>
                  )}
                </td>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-px bg-hairline hairline">
              ...
              <div className="bg-background p-4">
                ...
                <div className="mt-2 font-mono text-lg">{selected.callsLast24h.toLocaleString()}</div>
              </div>
              <div className="bg-background p-4">
                <div className="micro-label">— Status</div>
                <div className="mt-2 font-mono text-sm">{selected.enabled ? "Enabled" : "Disabled"}</div>
              </div>
              <div className="bg-background p-4">
