    ...
    <header className="fixed top-0 inset-x-0 z-40 hairline-b bg-background/80 backdrop-blur-md">
      <div className="container-editorial flex items-center justify-between h-14">
        ...
        <nav className="flex items-center gap-2">
          <a href="#lifecycle" className="pill text-[0.7rem] hidden sm:inline-flex">Spec</a>
          <a href="#architecture" className="pill text-[0.7rem] hidden md:inline-flex">Architecture</a>
          <Link to="/dashboard" className="pill text-[0.7rem]">Dashboard</Link>
    ...
    <section className="relative min-h-[100svh] hairline-b overflow-hidden">
      ...
      <div className="relative container-editorial pt-32 pb-16 min-h-[100svh] flex flex-col">
        ...
        <div className="mt-16 hairline-t pt-4 grid grid-cols-3 gap-4 font-mono text-[0.7rem] text-muted-foreground">
          <div>STATUS — <span className="text-status-success">● devnet live</span></div>
...
const lifecycleSteps = [
  ...
  { n: "02", title: "402 Challenge", code: `HTTP 402 Payment Required\nx-pay-scheme: x402.sol.usdc.v1\namount: 0.005 USDC` },
  { n: "03", title: "Pay USDC", code: `signTransaction({\n  to: payoutWallet,\n  lamports: 5_000\n})` },
  { n: "04", title: "Verify", code: `POST /v1/verify\n{ "sig": "5KJp7z…", "req": "req_8KpL" }` },
  { n: "05", title: "Execute", code: `→ MCP tool dispatch\nresult: { ok: true }` },
  { n: "06", title: "Receipt", code: `{ "id": "rcpt_8KpL",\n  "amount": "0.005 USDC",\n  "block": 312_887_142 }` },
];
      ...
      <div className="container-editorial py-20">
        <div className="grid grid-cols-12 gap-8 mb-12">
          ...
          <p className="col-span-12 md:col-span-6 md:col-start-7 text-foreground/70 self-end">
            Every paid tool call follows the same path. The gateway handles the 402,
            the verifier checks the on-chain transfer, the MCP server only runs
            after settlement.
          </p>
        </div>
    ...
    <section className="hairline-b">
      <div className="container-editorial py-24 grid grid-cols-12 gap-8">
        ...
        <p className="col-span-12 md:col-span-8 text-2xl md:text-4xl tracking-tight leading-[1.15] text-foreground/85">
          ...
          <span className="text-foreground">Price tools individually.</span>{" "}
          Settle on Solana.{" "}
          <span className="text-foreground">Inspect every receipt</span> — and
          every failed payment — without writing a billing system.
        </p>
      </div>
    </section>
  );
...
const features = [
  { label: "Pricing", title: "Tool-level pricing", desc: "Set a USDC price per tool. Free tools stay free.", code: `tools.set("web.search", "0.005")` },
  { label: "Settlement", title: "USDC on Solana", desc: "Sub-second confirmation on devnet. SPL Token transfers.", code: `network: "devnet"\nmint:    EPjFW…UsdcZ` },
  { label: "Integration", title: "Drop-in MCP wrapper", desc: "One middleware in front of your existing MCP server.", code: `app.Use(x402.Middleware(cfg))` },
  { label: "Receipts", title: "Signed receipts", desc: "Every paid call gets a verifiable receipt with tx signature.", code: `GET /v1/dashboard/receipts` },
  { label: "Failures", title: "Failed payment visibility", desc: "Insufficient funds, bad sigs, timeouts — all surfaced.", code: `reason: "signature_invalid"` },
  { label: "Devflow", title: "Devnet first", desc: "Build, test, and inspect locally before going live.", code: `RPC = api.devnet.solana.com` },
];
      ...
      <div className="container-editorial py-20">
        <div className="grid grid-cols-12 gap-8 mb-12">
          <div className="col-span-12 md:col-span-5">
            ...
            <h2 className="display-md mt-4">Six components.<br />One signed lifecycle.</h2>
          </div>
          <p className="col-span-12 md:col-span-5 md:col-start-8 text-foreground/70 self-end">
            The gateway never executes a tool until the verifier confirms the
            transfer on Solana. Receipts are signed and queryable.
          </p>
        </div>
...
const useCases = [
  { tag: "01 / API", title: "Paid AI tool APIs", desc: "Charge agents per call. No keys, no Stripe, no monthly plan." },
  { tag: "02 / Data", title: "Metered data sources", desc: "Sell access to embeddings, scrapers, indices — by the request." },
  { tag: "03 / Skills", title: "Premium agent skills", desc: "Gate expensive tools (image gen, code exec) behind USDC." },
];
      ...
      <div className="container-editorial py-20 grid grid-cols-12 gap-8">
        <div className="col-span-12 md:col-span-4">
          ...
          <p className="mt-6 text-foreground/70">
            The 402 challenge is a normal HTTP response. The verify call takes a
            signature and a request id. That's the entire surface.
          </p>
  ...
  return (
    <section className="hairline-b relative overflow-hidden">
      <div className="container-editorial py-32 text-center">
        ...
        <h2 className="display-xl mt-8">SHIP A PAID<br />MCP TODAY.</h2>
        ...
        {[
          ...
          <div key={c.h}>
            ...
            <ul className="mt-4 space-y-2 text-sm">
              {c.l.map((it) => (
                <li key={it}>
                  <a href="#" className="hover:text-foreground text-foreground/70 transition-colors">
                    {it}
                  </a>
                </li>
              ))}
